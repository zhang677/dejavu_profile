...
def gather_gemv_kernel2(
    ...
    """
    Kernel for computing Y = ACTIVATION(A[IDX, :] @ X) * (B[IDX, :] @ X), where A and B are 
    dense matrices with Z rows and N columns. We don't support batch of the input X. 
    We will not check that the indices are valid, for performance reason.
    - Input X has shape (BATCHSIZE, N)
    - A and B have shape (Z, N)
    - IDX has shape (M), where M is the number of non-zero rows in A
    - Output has shape (BATCHSIZE, M)
    """
    start_m = tl.program_id(0)
    ...
    idx = tl.load(IDX, mask=rm < M, other=0)
    A = A + (idx[:, None] * stride_am + rn[None, :])
    B = B + (idx[:, None] * stride_am + rn[None, :])
    X = X + rn
    ...
    if BATCHSIZE == 1:
        ...
        for n in range(N, 0, -BLOCK_N):
            a = tl.load(A) if EVEN_N else tl.load(A, mask=rn[None, :] < n, other=0.0)
            b = tl.load(B) if EVEN_N else tl.load(B, mask=rn[None, :] < n, other=0.0)
            x0 = tl.load(X) if EVEN_N else tl.load(X, mask=rn < n, other=0.0)
            acc0 += tl.sum(a.to(tl.float32) * x0.to(tl.float32)[None, :], 1)
            acc1 += tl.sum(b.to(tl.float32) * x0.to(tl.float32)[None, :], 1)
            A += BLOCK_N
            B += BLOCK_N
            X += BLOCK_N
        acc0 = apply_activation(acc0, ACTIVATION=ACTIVATION)
...
def gather_gemv2(
    ...
    :param x: input tensor, (batch, N)
    :param wgate: gate weight matrix, (Z, N)
    ...
    x = x.contiguous()
    if wgate.stride(1) > 1:
        wgate = wgate.contiguous()
    assert (
        x.dtype == wgate.dtype
    ), f"Input and weight must have the same dtype, got {x.dtype} and {wgate.dtype}"
    if wup.stride(1) > 1:
        wup = wup.contiguous()
    assert (
        x.dtype == wup.dtype
...
def mistral_mlp_sparse(x, Wgate, Wup, Wdownt, idx, activation="relu"):
    """
    x: (batch, d)
    Wgate: (3.5d, d)
    Wup: (3.5d, d)
    Wdownt: (3.5d, d). This is the transpose of W2. We want this to be contiguous for fast loading.
    idx: (M)
    """
